...
@app.task(base=StandardTask, bind=True, name='list_all_user_iots_task')
def list_all_user_iots_task(self, user_data):
    headers = {'X-Correlation-ID': self.cid}
    # Log para confirmar que a rota foi chamada
    self.logger.info(f"Dashboard: Listando TODAS as portas (Acesso Liberado).")

    try:
        # --- A MUDANÇA ESTÁ AQUI ---
        # Não passamos mais o 'user_id' nos params.
        # Ao não enviar 'user_id', o servico-banco-dados retorna TUDO.
        params = {'status': 'true'} 
        ...
        self.logger.error(f"Erro ao listar todos os IoTs: {e}")
        return {'error': str(e)}